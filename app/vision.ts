...
function updateTrack(
  ...
  const updated: TrackRecord = {
    ...
    expansionRate:
      previous.expansionRate * 0.62 + rawExpansion * 0.38,
    updatedAt: now,
    history,
  };
...
function assessRisk(
  ...
  const closingFast = track.expansionRate > 0.08;
  let risk: RiskLevel = isInformational ? "info" : "safe";

  if (isHazard && (inPath || (predictedInPath && closingFast))) {
    if (
      ttc <= profile.dangerTtc ||
      (distance <= profile.dangerDistance && (nearBottom || closingFast)) ||
      (nearBottom && distance <= profile.dangerDistance * 1.35)
    ) {
      risk = "danger";
    } else if (
      distance <= profile.warningDistance ||
  ...
  const rank = risk === "danger" ? 3 : risk === "watch" ? 2 : risk === "safe" ? 1 : 0;
  const severity =
    rank * 100 +
    (inPath ? 24 : predictedInPath ? 12 : 0) +
    Math.max(0, profile.warningDistance - distance) +
    Math.max(0, profile.warningTtc - ttc) * 8;

  return { risk, ttc, inPath, predictedInPath, severity };
}
...
export function analyzeDetections(
  ...
  for (const box of boxes) {
    ...
    const distance = estimateDistance(box, width, height, mode);
    ...
    detections.push({
      ...
      direction: directionFor(track.cx, width),
